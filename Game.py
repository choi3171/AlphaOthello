...
class Quoridor:
    def __init__(self):
        self.game = qe.Quoridor()
        self.action_size = 121 # 49 + 36 + 36
    ...
    def get_value_and_terminated(self, state, count):
        return self.game.get_value_and_terminated(state, count)

    def get_encoded_state(self, state):
        raw = self.game.get_encoded_state(state)
        return np.array(raw, dtype=np.float32).reshape(6, 7, 7)
